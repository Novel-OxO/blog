...
**쌓임 맥락**은 z-index가 적용되는 **독립적인 레이어 그룹**이다. 같은 쌓임 맥락 안에서만 z-index 비교가 의미 있으며, 서로 다른 쌓임 맥락에 속한 요소끼리는 **부모의 z-index로 승패가 결정**된다.

```mermaid
flowchart TB
  subgraph ctxA["쌓임 맥락 A (z-index: 1)"]
    A1["자식 z-index: 999"]
    A2["자식 z-index: 500"]
  end

  subgraph ctxB["쌓임 맥락 B (z-index: 2)"]
    B1["자식 z-index: 1"]
    B2["자식 z-index: 0"]
  end

  result["결과: B가 A보다 위에 표시<br/>A의 자식이 999여도 B의 자식(1)보다 아래!"]
  ctxA --> result
  ctxB --> result
...
무분별하게 큰 값을 사용하면 z-index 전쟁이 발생한다. CSS 변수로 체계적으로 관리하는 것이 좋다.
